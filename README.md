...
联网只发生在三个时刻：**冷启动**、**定时（60 秒）**、**手动点刷新**（侧栏账号行上的按钮）。
每次都是同样两步——先 `labels.list` 拉标签定义，再 `history.list` 同步
邮件变化。history 已经带回了变动的标签，本地按增删应用即可，不必逐封回头去问。
...
Sources/MgmailApp/
  ...
  UI/       三栏界面、会话列表、正文渲染、标签编辑、活动栏与活动窗口
