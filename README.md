...
- **渠道**（设置 → 更新）：**正式版**只跟 tag（`releases/latest/download/manifest.json`，
  GitHub 会跳过预发布）；**开发版**跟 `main-latest`，同时也看正式版那份，两份里版本高的算数。
  打了 tag 的提交本身也在 main 上，开发版理应是正式版的超集；而且 `main-latest` 是合并那一刻
  构建的，tag 还没打，写进去的版本号是上一个 tag 的——之后在同一提交上打的 tag 不会让它重建，
  只看它就会拿着 1.13.3（构建 49）查不到同一构建的正式版 1.13.4。
...
- 弹窗（有新版 / 已是最新 / 下载好了）挂在当前窗口上当 sheet，窗口在哪块屏它就在哪块屏；
  一个窗口都没开时才独立弹出，放到鼠标所在的屏幕上。这套出场规则在 `UI/ModalPresentation.swift`
  里（`ModalHost`），附件下载 / 存 .eml / 撰写添加附件的系统面板也走它——独立 `runModal()`
  的面板会落到带菜单栏的那块屏上，主窗口在副屏时看起来就是「点了没反应」。
- 设置窗口（⌘,）是独立窗口挂不成 sheet，而 SwiftUI 会记住它上次关在哪块屏。它每次出来时
  看一眼落在哪：不在正在用的那扇窗所在的屏上就挪过去、居中盖在那扇窗上，之后手动拖走不再管
  （`UI/FollowMainScreen.swift`）。
